export const menu = [
    {
        key: 'branch', title: '关于分会', icon: 'appstore',
        sub: [
            { key: '/child/branch/brachInfo', title: '分会介绍', icon: '', },
            { key: '/child/branch/organization', title: '组织结构', icon: '', },
            { key: '/child/branch/regulation', title: '分会章程', icon: '', },
            { key: '/child/branch/history', title: '发展历程', icon: '', },
            { key: '/child/branch/event', title: '大事记', icon: '', },
            { key: '/child/branch/documents', title: '相关文件', icon: '', }
        ],
    },
    {
        key: 'academic', title: '学术交流', icon: 'appstore',
        sub: [
            { key: '/child/academic/dynamicsWork', title: '工作动态', icon: '', },
            { key: '/child/academic/eventsNotice', title: '活动公告', icon: '', },
            { key: '/child/academic/notices', title: '通知', icon: '', },
            { key: '/child/academic/skillDynamics', title: '技术动态', icon: '', }
        ],
    },
    {
        key: 'award', title: '表彰奖励', icon: 'appstore',
        sub: [
            { key: '/child/award/rewardDynamics', title: '奖励动态', icon: '', },
            { key: '/child/award/rewardRecommendation', title: '奖励推荐', icon: '', },
            { key: '/child/award/awardsIntroduced', title: '奖项介绍', icon: '', },
            // { key: '/child/award/YearendAwards', title: '历年获奖情况', icon: '', },
            { key: '/child/award/Sponsors', title: '赞助单位', icon: '', }
        ],
    },
    {
        key: 'certification', title: '资质认证', icon: 'appstore',
        sub: [
            { key: '/child/certification/titlereview', title: '职称评审', icon: '', },
            { key: '/child/certification/reviewRule', title: '评审细则', icon: '', },
            { key: '/child/certification/application', title: '职称申请', icon: '', },
            { key: '/child/certification/reviewNotice', title: '评审公告', icon: '', }
        ],
    },
    {
        key: 'SciencePark', title: '科普园地', icon: 'appstore',
        sub: [
            { key: '/child/SciencePark/scienceDynamics', title: '科普动态', icon: '', },
            { key: '/child/SciencePark/publication', title: '出版刊物', icon: '', },
        ],
    },
    {
        key: 'membershipService', title: '会员服务', icon: 'appstore',
        sub: [
            { key: '/child/membershipService/titlereview', title: '职称评审', icon: '', },
            { key: '/child/membershipService/reviewRule', title: '评审细则', icon: '', },
            { key: '/child/membershipService/titleApplication', title: '职称申请', icon: '', },
            { key: '/child/membershipService/reviewNotice', title: '评审公告', icon: '', }
        ],
    },
    {
        key: 'digitalData', title: '数字资料', icon: 'appstore',
        sub: [
            { key: '/child/digitalData/academicPapers', title: '学术论文', icon: '', },
            { key: '/child/digitalData/conference', title: '会议活动', icon: '', },
            { key: '/child/digitalData/powerPoint', title: 'PPT', icon: '', },
            { key: '/child/digitalData/video', title: '视频', icon: '', },
            { key: '/child/digitalData/atlas', title: '图集', icon: '', }
        ],
    }
];